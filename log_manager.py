...
class User():
    ...
    def save(self):
        if self.password:
            return "INSERT INTO users(username, password, active, admin) VALUES('%s', '%s', '%s', '%s')" % (self.username, self.password, self.active, self.admin)
    
    def set_password(self):
        if self.password and self.id:
            return "UPDATE users SET `password`='%s' WHERE id=%s" % (self.password, self.id)
        
    def set_active(self):
        if self.id:
            return "UPDATE users SET `active`='%s' WHERE id=%s" % ("True", self.id)
    
    def set_inactive(self):
        if self.id:
            return "UPDATE users SET `active`='%s' WHERE id=%s" % ("False", self.id)
        
    def set_as_admin(self):
        if self.id:
            return "UPDATE users SET `admin`='%s' WHERE id=%s" % (self.id, "True")
        
    def set_not_as_admin(self):
        if self.id:
            return "UPDATE users SET `admin`='%s' WHERE id=%s" % (self.id, "False")
